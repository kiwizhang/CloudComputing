...
select c_city, s_city, d_year, sum(lo_revenue) as revenue from 
lineorder_opt 
JOIN (select * from dwdate_opt where d_yearmonth = 'Dec1997') as dwdate_temp on lo_orderdate = d_datekey 
JOIN (select * from customer_opt where (c_city='UNITED KI1' or c_city='UNITED KI5')) as customer_temp on lo_custkey = c_custkey
JOIN (select * from supplier_opt where (s_city='UNITED KI1' or s_city='UNITED KI5')) as supplier_temp on lo_suppkey = s_suppkey
group by c_city, s_city, d_year order by d_year asc, revenue desc limit 5;
-- end query3_opt
