...
subscripid  = sys.argv[2]
endpoint_uri = sys.argv[3]
app_id = sys.argv[4]
app_secret_key = sys.argv[5]
pwd = sys.argv[6]
andrewid = sys.argv[7]
...
def demo_lg():
    ...
    req_test = requests.get(submit_url)
    # req = urllib2.Request(submit_url)
    # response = urllib2.urlopen(req)
    the_page = req_test.text
    while ("Invalid" in the_page):
        req_test = requests.get(submit_url)
...
def launch_dc():
    ...
    testpage = req_test.text

    while ("Invalid" in testpage):
        req_test = requests.get(test_url)
        testpage = req_test.text
    print "testpage:" + testpage
    ...
    while req_id is None:
        req_id = requests.get(testid_url)
        req_id_content = req_id.text
        test_number = req_id_content.split("test.")[-1].split(".")[0]
...
if __name__ == '__main__':
    ...
    while (rps_count < 3000 and status != "finished"):
        ...
        rps_count = 0
        launch_dc()
        urllist.append(log_url)
        for log_url in urllist:
            ...
            log_page = log_req.text
            
            while log_req is None or not log_page or log_page.find("[Minute 1]") == -1:
                time.sleep(5)
                print "trying"
                log_req = requests.get(log_url)
                log_page = log_req.text
            ...
            print "log_page: " + log_page
            print log_page.split("=")[-1]
            rps_count = rps_count + float(log_page.split("=")[-1].strip())
        print 'succeed!'
