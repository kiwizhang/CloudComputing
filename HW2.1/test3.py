...
#     req_test = None
#     response_test = None
#     while req_test is None or response_test is None:
#         try:
...
#             pass
    ...
    #     try:
    #         req_log = urllib2.Request(log_url)
    #         response = urllib2.urlopen(req)
    #     except:
    #         pass
    ...
    while (rps_count < 3000 and status != "finished"):
        ...
        for log_url in urllist:
            ...
            while response is None:
                try:
                    req_log = urllib2.Request(log_url)
                    response = urllib2.urlopen(req_log)
                except:
                    pass
            ...
            for i in reversed(xrange(len(pageline))):
                if pageline[i].startswith("[Minute"):
                    rps_count = rps_count + float(pageline[i+1].split("=")[1])
                    print rps_count
